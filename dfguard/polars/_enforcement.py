"""Schema enforcement for the Polars backend."""
...
def _raise_schema_mismatch(
    ...
    if isinstance(value, pl.LazyFrame):
        actual_str = ", ".join(f"{c}:{d}" for c, d in value.collect_schema().items())
    elif isinstance(value, pl.DataFrame):
        actual_str = ", ".join(f"{c}:{d}" for c, d in value.schema.items())
